...
class KnowledgeManagerAgent:
    ...
    def teach_from_unanswered(self, business_id: str, unanswered_queries: List[Dict]) -> List[Dict]:
        ...
        prompt = (
            ...
            f"{queries_text}\n\n"
            "### TASK ###\n"
            "Generate clear, helpful FAQ-style answers. You MUST use your domain expertise to define terms "
            "found in the business name (e.g., explaining 'Sawan' if the business is 'Sawan Indian Cuisine') "
            "or terms clearly related to the industry (ingredients, cuisine styles, cultural references) "
            "even if they are not explicitly mentioned in the original documents.\n\n"
            "### FAITHFULNESS & ACCURACY (CRITICAL) ###\n"
            "While using domain expertise, maintain extreme factual accuracy. DO NOT use generic 'catch-all' "
            "definitions. For example: If asked about 'Andhra', do NOT say it's famous for 'dosa and sambar' "
            "(which are generic to all of South India). Instead, mention specific Andhra traits like "
            "'bold, spicy, tangy, Gongura, or Vepudu'. If you are unsure of the specific unique traits, "
            "state that the information is not explicitly listed rather than providing a generic hallucination.\n\n"
            "CRITICAL: If the question asks for a specific operational service (like 'Lunch Buffet', 'Valet Parking', "
            "'Childcare') and it is NOT a standard common offering in this industry, EXPLICITLY state that the service "
            "is currently not listed in our records to avoid false promises. Do NOT guess operational details like "
            "prices, hours, or specific policies if you don't know them.\n\n"
            "### OUTPUT FORMAT ###\n"
            "Produce a valid JSON list of objects. Each object must have 'question' and 'suggested_answer'.\n"
            "If you have no information for a query and it is not domain-related, set suggested_answer to 'Information not found in current records.'\n"
            "ONLY return the JSON list. No other text."
        )
